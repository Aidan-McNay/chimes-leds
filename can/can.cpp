...
unsigned short  rx_packet_unstuffed[MAX_PACKET_LEN >> 1]       = { 0 };
...
// Packet transmission
unsigned short CAN::getBitShort( unsigned short* shorty,
...
void CAN::modifyBitShort( unsigned short* shorty, unsigned char bitnum,
...
void CAN::sendPacket()
{
  ...
  while ( checksum == 0xFFFF ) {
    ...
    for ( int i = 0; i < ( payload_len + 2 ); i++ ) {
      ...
      checksum =
          culCalcCRC( ( tx_packet_unstuffed[i] ) & 0xFF, checksum );
    }
  }

  // Load checksum
  tx_packet_unstuffed[payload_len + 2] = checksum;
  // Load EOF
  tx_packet_unstuffed[payload_len + 2 + 1] = 0xFFFF;
...
void CAN::unBitStuff(unsigned char * stuffed, unsigned short * unstuffed) {
    // Clear the buffer
    ...
    while ( ( *( stuffed + stuffed_index ) != 0xFF ) &&
        ...
        if ( bit_run_len < 5 ) {
            modifyBitShort( unstuffed + unstuffed_index, unstuffed_bit, new_val );
            unstuffed_bit   = ( unstuffed_bit < 15 ) ? ( unstuffed_bit + 1 ) : 0;
...
unsigned char CAN::attemptPacketReceive() {
    ...
    if ((rx_packet_unstuffed[0] != my_arbitration) &&
        ...
        printf("Arbitration check failed!\n");
        return 0;
    }
    ...
    for (int k = 0; k < (len >> 1) + 4; k++) {
        printf("rx_packet_unstuffed[%d] = 0x%04x\n", k, rx_packet_unstuffed[k]);
    }

    // Compute and check checksum
    unsigned short checksum = CRC_INIT;  // Init value for CRC calculation
    for (int i = 0; i < ((len >> 1) + 2); i++) {
        checksum = culCalcCRC((rx_packet_unstuffed[i] >> 8) & 0xFF, checksum);
...
// Deive ISR
// resets the DMA channel when overrun on the RX DMA channel (a new node
// joins the network)
void CAN::dma_handler()
{
  ...
  dma_channel_set_write_addr( dma_chan_1, rx_packet_stuffed_pointer,
                              true );
}
...
void CAN::setupIdleCheck()
{
  ...
  dma_channel_configure(
      ...
      &pio_0->txf[can_idle_check_sm],  // write address (idle check PIO TX
                                       // FIFO)
  ...
  dma_channel_configure( dma_chan_3,  // Channel to be configured
...
inline void CAN::resetTransmitter()
{
  ...
  dma_channel_set_read_addr( dma_chan_0, tx_packet_stuffed_pointer,
                             false );
  // WHY IS THIS NECESSARY? Did not need this until I added the
  // transcievers
  sleep_us( 10 );
}
