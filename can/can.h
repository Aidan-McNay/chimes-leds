...
#define CAN_TX 16  // CAN RX is at CAN_TX+1
#define LED_PIN 25
...
class CAN {
 ...
 public:
  ...
  void set_payload( unsigned short* new_payload, int word_len )
  {
    payload_len = word_len;
    for (int i = 0; i < word_len; i++) {
        payload[i] = new_payload[i];
    }
  }
  unsigned short get_my_rbitration()
  {
  ...
  void          modifyBitChar( unsigned char* byte, unsigned char bitnum,
                               unsigned char value );
  void unBitStuff( unsigned char* stuffed, unsigned short* unstuffed );
 ...
 protected:
  ...
  unsigned short payload[MAX_PAYLOAD_SIZE >> 1];
